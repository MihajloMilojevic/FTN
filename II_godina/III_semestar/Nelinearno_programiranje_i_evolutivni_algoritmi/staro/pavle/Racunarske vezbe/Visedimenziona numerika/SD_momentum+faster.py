...
result_sdm = steepest_descent_with_momentum_v(gradf, x0=[0, 0], gamma=0.1, epsilon=1e-6, omega=0.2, n=200)
# Povecavamo uticaj momenta preko omega - brze se dolazi do boljeg rezultata
result1_sdmf = steepest_descent_with_momentum_v(gradf, x0=[0, 0], gamma=0.1, epsilon=1e-6, omega=0.7, n=75)
print(result_sdm[-1])
print(result1_sdmf[-1])

plot_run(f, result_sdm, np.arange(-22, 12, 0.1), np.arange(-22, 12, 0.1), label="SD Momentum")
plot_run(f, result1_sdmf, np.arange(-22, 12, 0.1), np.arange(-22, 12, 0.1), label="SD momentum -- faster")