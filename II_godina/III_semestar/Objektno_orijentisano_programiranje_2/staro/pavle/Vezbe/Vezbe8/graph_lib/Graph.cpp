...
void Function::reset(double r1, double r2,
    int count, Point xy, double xscale, double yscale, Fct f, double mov)
{
    double dist = (r2 - r1) / count;
    double r = r1;
    for (int i = 0; i < count - 1; ++i) {
        set_point(i, Point(xy.x + int(r * xscale), xy.y - int(f(r - mov) * yscale)));
        r += dist;
    }
...
MyFunction::MyFunction(Fct f, double r1, double r2, Point xy,
    int count, double xscale, double yscale)
    // graph f(x) for x in [r1:r2) using count line segments with (0,0) displayed at xy
    // x coordinates are scaled by xscale and y coordinates scaled by yscale
{
