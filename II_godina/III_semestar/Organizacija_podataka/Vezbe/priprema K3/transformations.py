...
def square_transformation(key: int, B: int, v: int, p: int) -> int:
    ...
    digits = list(str(key ** 2).zfill(2 * p - 1))
    ...
    central_value = int("".join(central_digits))
...
def overlapping_transformation(key: int, B: int, v: int, p: int) -> int:
    key = key + pow(v, p + 1) #12 10012
    n = math.ceil(math.log(B, v)) 
    numberOfSegments = math.ceil(p / n) 
    power = 1 
    sum = 0 
    ...
    segment_values = [int("".join(segment)) for segment in segments]
...
if __name__ == "__main__":
    ...
    # Sa predavanja, prezentacija 8, slajd 34, smanjeno za jedan da indeks počneo od 0, a ne 1
    assert square_transformation(37, 150, 10, 3) == 20
    # Sa predavanja, prezentacija 8, slajd 36, smanjeno za jedan da indeks počneo od 0, a ne 1
