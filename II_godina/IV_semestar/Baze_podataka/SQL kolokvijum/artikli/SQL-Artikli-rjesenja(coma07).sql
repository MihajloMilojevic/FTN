...
select Faktura.BRFAK, Kupac.IDKUP from Faktura, Kupac where Faktura.IDKUP = Kupac.IDKUP and Faktura.IDKUP in (1, 2) order by Faktura.BRFAK desc;
...
select Stavka.RBRST, Artikal.JCENA from Stavka, Artikal where Stavka.IDART = Artikal.IDART and Stavka.BRFAK = 4 and Artikal.NAZART like '%a%';
select Stavka.RBRST, Artikal.JCENA from Stavka natural join Artikal where Stavka.BRFAK = 4 and Artikal.NAZART like '%a%';
