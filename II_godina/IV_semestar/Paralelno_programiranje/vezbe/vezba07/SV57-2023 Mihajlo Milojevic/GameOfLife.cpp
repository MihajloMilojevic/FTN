...
int GameOfLife::nextIterParallel(int _row, int _col, int _width, int _height) {
	int rebornCells = 0;
	if (_width < grain_size || _height < grain_size) {
		for (int row = _row; row < _row + _height; row++) {
		...
		int a = 0, b = 0, c = 0, d = 0;
		int left = _width / 2;
		int right = _width - left;

		int top = _height / 2;
		int bottom = _height - top;

		t.run([&] {a = nextIterParallel(_row, _col, left, top); }); // top left
		t.run([&] {b = nextIterParallel(_row + top, _col, left, bottom); }); // bottom left
		t.run([&] {c = nextIterParallel(_row, _col + left, right, top); }); // top right
		t.run([&] {d = nextIterParallel(_row + top, _col + left, right, bottom); }); // bottom right
		t.wait();
		rebornCells = a + b + c + d;
	}
...
class RebirthCounter {
public:
	long count = 0;
	unsigned char* prev = nullptr;
	unsigned char* curr = nullptr;
	RebirthCounter(unsigned char* prev, unsigned char* curr) : prev(prev), curr(curr) {}
	RebirthCounter(RebirthCounter& other, split) : prev(other.prev), curr(other.curr) {}
	void operator()(const blocked_range<long>& r) {
		for (long i = r.begin(); i != r.end(); ++i) {
			if (prev[i] == 0 && curr[i] == 1) ++count;
		}
	}
	void join(const RebirthCounter& other) {
		count += other.count;
	}
};


int GameOfLife::calculateReborn() {
	RebirthCounter counter(nextIter, buffer);
	parallel_reduce(
		blocked_range<long>(0, width * height),
		counter
	);
